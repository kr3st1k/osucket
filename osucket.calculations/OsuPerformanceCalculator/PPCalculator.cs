...
namespace OsuPerformanceCalculator
{
	public abstract class PPCalculator
	{
		...
		public double Calculate(WorkingBeatmap workingBeatmap, double endTime, double accuracy, int combo, int miss = 0,
			int meh = 0, string[] Mods = null, int score = 0)
		{
		...
		public double Calculate(WorkingBeatmap workingBeatmap, double accuracy, int combo, int miss = 0, int meh = 0,
			string[] Mods = null, int score = 0)
		{
			...
			PerformanceCalculator performanceCalculator =
				Ruleset.CreatePerformanceCalculator(workingBeatmap, scoreInfo);
		...
		public int GetMaxCombo(IBeatmap beatmap)
		{
			return GetMaxCombo(beatmap.HitObjects);
		}

		protected abstract int GetMaxCombo(IReadOnlyList<HitObject> hitObjects);

		public double GetTimeAtHits(IBeatmap beatmap, int hits)
		{
			return GetTimeAtHits(beatmap.HitObjects, hits);
		}

		protected abstract double GetTimeAtHits(IReadOnlyList<HitObject> hitObjects, int hits);

		protected abstract Dictionary<HitResult, int> GenerateHitResults(double accuracy,
			IReadOnlyList<HitObject> hitObjects, int countMiss, int countMeh);
